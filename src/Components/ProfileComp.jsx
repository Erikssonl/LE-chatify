...
const ProfileComp = () => {
  const { decodedJwt, updateUserData, deleteUser, handleFileChange, imgUrl } = useContext(ChatContext)
  
  const navigate = useNavigate()
  const [inputUsername, setInputUsername] = useState(decodedJwt.user || "")
  const [uInputEnable, setUinputEnabled] = useState(false)
  const [inputEmail, setInputEmail] = useState(decodedJwt.email || "")
  const [eInputEnable, setEinpuEnable] = useState(false)
  const [userImg, setUserImg] = useState(decodedJwt.avatar)
  const [saveStatus, setSaveStatus] = useState(null);
  const [saveMessage, setSaveMessage] = useState('');
  const [deleteMessage, setDeleteMessage] = useState('');

  const toggleUsernameEdit = () => {
    setUinputEnabled(!uInputEnable);
  }
  const toggleEmailEdit = () => {
    setEinpuEnable(!eInputEnable);
  }

  useEffect(() => {
    setInputUsername(decodedJwt.user || "");
    setInputEmail(decodedJwt.email || "");
  }, [decodedJwt]);
  ...
  const handleSaveChanges = async () => {
    ...
    if (result.success) {
      ...
      const { user, email, avatar } = result.updatedUser;
      setInputUsername(user);
      setInputEmail(email);
      setUserImg(avatar);
    } else {
      setSaveStatus(false);
      setSaveMessage(result.message || 'Failed to update user.')
    }
  };
  ...
  return (
    <div >
      ...
      <div className="card bg-base-100 p-7 flex flex-auto justify-center items-center flex-row">
          <div>
            <figure className="flex-1 mr-0 mb-4" >
              <img
                src={userImg || decodedJwt.avatar}
                alt="user avatar" />
            </figure>
            <input 
              type="file" 
          ...
          <div className="card-body">
            <div className={style.inputWrap}>
              <input
                ...
                disabled={!uInputEnable}
                className="input input-bordered w-full max-w-xs"
              />
              <button className="btn btn-primary cursor-pointer" onClick={toggleUsernameEdit}> 
            ...
            <div className={style.inputWrap}>
              <input
                ...
                disabled={!eInputEnable}
                className="input input-bordered w-full max-w-xs"
              />
              <button className="btn btn-primary cursor-pointer" onClick={toggleEmailEdit}> 
                <svg xmlns="http://www.w3.org/2000/svg" 
                  width="15" height="15" viewBox="0 0 15 15" 
                  fill="none"
                  >
