...
const SignIn = () => {
  const { username, setUsername, password, setPassword, signIn } = useContext(ChatContext)
  const [errorMessage, setErrorMessage] = useState('')
  const navigate = useNavigate()

  const handleSignIn = async () => {
    try {
      await signIn(username, password, () => navigate('/chat'));
    } catch (error) {
      setErrorMessage('Sign in failed. Check your username or password');
    }
  };
